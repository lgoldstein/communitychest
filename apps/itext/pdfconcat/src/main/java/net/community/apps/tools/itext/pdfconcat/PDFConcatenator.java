...
public class PDFConcatenator extends SwingWorker<Void,File> {
    ...
    {
        if ((null == (_f=f))
         || (null == (_fl=fl)) || (fl.size() <= 0)
         || (null == (_outFile=outFile)) || (outFile.length() <= 0))
    ...
    protected Void doInBackground () throws Exception
    {
        ...
        for (final Iterator<? extends File>        fi=((null == fl) || (fl.size() <= 0)) ? null : fl.iterator();
             (fi != null) && fi.hasNext() && (!isCancelled());
             )
        {
