...
public abstract class EntryThermometerPlot extends ThermometerPlot {
	...
	{
		if (null == (_tsType=tsType))
			throw new IllegalArgumentException("No TS type specified");

		{
			final Map.Entry<Number,Number>	r=getRangeBoundaries();
			if (r != null)
				setRange(r.getKey().doubleValue(), r.getValue().doubleValue());
		}
	...
	public static final EntryThermometerPlot createEntryThermometerPlot (
			final TimeSeriesTypeCase tsType)
	{
		...
		switch(tsType)
		{
			case DIA	: return new DiaThermometerPlot();
			case SIS	: return new SisThermometerPlot();
			case HR		: return new HRThermometerPlot();
			default		:
