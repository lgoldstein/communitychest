...
public class WCFileAttrsCellRenderer extends FileAttrsCellRenderer {
	...
	/*
	 * @see net.community.chest.ui.components.table.file.FileAttrsCellRenderer#getIcon(java.io.File)
	 */
	@Override
	protected Icon getIcon (File f)
	{
		try
		{
			// TODO use recursive status for folders
			...
			if (isModifiedStatus(ctxEnum))
				return getContentIcon(ctxEnum);
			...
			if (isModifiedStatus(prpEnum))
				return getPropertiesIcon(prpEnum);
