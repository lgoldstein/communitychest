...
public abstract class AbstractDaoTestSupport extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    protected <T extends Identified,DAO extends IdentifiedCommonOperationsDao<T>>
            List<T> runIdentifiedByIdFinder (final DAO dao) {
        return runIdentifiedInstanceFinderTest(dao, (IdentifiedInstanceFinder) BY_ID_FINDER);
    }

    protected <T extends Identified,DAO extends IdentifiedCommonOperationsDao<T>>
            List<T> runIdentifiedInstanceFinderTest (
                    final DAO dao, final IdentifiedInstanceFinder<T,DAO> finder) {
        return runInstanceFinderTest(dao, finder);
    }

    protected <ID extends Serializable,T,DAO extends CommonOperationsDao<T,ID>>
            List<T> runInstanceFinderTest (
                    final DAO dao, final InstanceFinder<ID,T,DAO> finder) {
    ...
    protected <T extends Identified & Located, DAO extends IdentifiedCommonOperationsDao<T>>
            void runLocatedInstanceFinderTest (final DAO                                        dao,
                                               final IdentifiedValuesListFinder<T,DAO,String>    finder) {
        ...
        for (final T value : valsList) {
            ...
            // should have found at least the current value
            ...
            for (final T entity : valsList) {
                ...
                if (StringUtils.containsIgnoreCase(entityLocation, qryLocation))
                    clcList.add(entity);
            }
    ...
    protected static final IdentifiedInstanceFinder<Identified,IdentifiedCommonOperationsDao<Identified>>
            BY_ID_FINDER=new IdentifiedInstanceFinder<Identified,IdentifiedCommonOperationsDao<Identified>>() {
                @Override
                public Identified findInstance(IdentifiedCommonOperationsDao<Identified> dao, Identified sourceInstance) {
                    if (sourceInstance == null)
                        return null;
                    else
                        return dao.findOne(sourceInstance.getId());
                }
            };
}
