 ...
 * @author lgoldstein
 */
public abstract class AbstractDTOMarshallingTestSupport<DTO extends IdentifiedDTO,L extends List<DTO>>
	...
	@Inject @Named("oxmMarshaller") protected Marshaller	_oxmMarshaller;
	@Inject @Named("oxmMarshaller") protected Unmarshaller	_oxmUnmarshaller;

	protected AbstractDTOMarshallingTestSupport(final Class<DTO> dtoClass, final Class<L> listClass)
	...
	protected List<DTO> runInstanceListMarshallingTest (final Marshaller marshal, final Unmarshaller unmarshal)
			throws InstantiationException, IllegalAccessException, InvocationTargetException, XmlMappingException, IOException, IntrospectionException {
		...
		if (!CollectionUtils.isEqualCollection(dtoList, resList))
			org.junit.Assert.fail("Mismatched lists contents - expected: " + dtoList + ", actual: " + resList);
		return dtoList;
	}

	protected DTO runInstanceMarshallingTest (final Marshaller marshal, final Unmarshaller unmarshal)
				throws InstantiationException, IllegalAccessException, XmlMappingException, IOException, IntrospectionException {
	...
	protected L wrapAsList (Collection<? extends DTO> dtoList)
