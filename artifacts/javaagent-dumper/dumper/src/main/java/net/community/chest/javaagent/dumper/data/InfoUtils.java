...
public final class InfoUtils {
    ...
    public static <A extends Appendable> A appendClassHeader (final A sb, final CharSequence name, final int mod, final URL url)
            throws IOException
    {
        return appendClassHeader(sb, name, mod, (url == null) ? null : url.toExternalForm());
    }

    public static <A extends Appendable> A appendClassHeader (final A sb, final CharSequence name, final int mod, final CharSequence location)
        throws IOException
    {
        ...
        if ((location != null) && (location.length() > 0))
            appendAttribute(sb, LOCATION_ATTR, location);
    ...
    public static final <A extends Appendable> A startMethod (final A sb, final String name)
            throws IOException
    {
    ...
    public static final String    VISIBILITY_ATTR="visibility",
                                ABSTRACT_ATTTR="abstract",
                                STATIC_ATTR="static",
                                FINAL_ATTR="final";
    public static final <A extends Appendable> A appendMethodModifiers (final A sb, final int mod)
        throws IOException
    {
    ...
    public static final String encodeMethodName (final String name)
    {
        ...
        if (((chFirst == '<') && (chLast != '>'))
         || ((chFirst != '<') && (chLast == '>')))
            throw new IllegalArgumentException("Mismatched c'tor name brackets: " + name);
    ...
    public static final String decodeMethodName (final String name)
    {
        ...
        if (startsWith)
            return "<" + name.substring(4, nLen - 4) + ">";
        else
            return name;
    }
    ...
    public static final int getModifiers (final Element elem)
    {
...

        {
            ...
            if ((vis == null) && (visAttr != null) && (visAttr.length() > 0))
                throw new IllegalArgumentException("Unkown visibility value: " + visAttr);
            mod |= vis.getModifier();
        }
    ...
    private static final int updateModifier (final Element elem, final String attrName, final int mod)
    {
        ...
        if (Boolean.parseBoolean(attrVal))
            return mod;
        else
            return 0;
    }
    ...
    public static final <A extends Appendable> A appendParamTypeAttribute (final A sb, final CharSequence type)
            throws IOException
    {
    ...
    public static final <A extends Appendable> A endMethod (final A sb, final boolean hasParams)
            throws IOException
    {
        if (hasParams)
            sb.append("</").append(METHOD_ELEMENT).append('>');
        else
            sb.append(" />");
        return sb;
    }

    public static final <A extends Appendable> A appendAttribute (final A sb, final CharSequence name, final CharSequence value)
            throws IOException
    {
        sb.append(' ')
          .append(name)
    ...
    public static final <A extends Appendable> A appendReturnTypeAttribute (final A sb, final CharSequence type)
            throws IOException
    {
