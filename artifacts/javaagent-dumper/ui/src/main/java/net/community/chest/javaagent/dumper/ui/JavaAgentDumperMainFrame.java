 ...
 */
class JavaAgentDumperMainFrame extends BaseMainFrame<ResourcesAnchor>
        implements SelectiblePackageInfoHandler, AspectGeneratorHandler {
    ...
    /*
     * @see net.community.apps.common.FilesLoadMainFrame#setMainMenuActionHandlers(net.community.chest.swing.component.menu.MenuItemExplorer, net.community.chest.swing.component.menu.MenuExplorer)
     */
    @Override
    ...
    void selectAllPackages ()
    {
        ...
        if ((paths != null) && (paths.length > 0))
            _pkgsTree.setSelectionPaths(paths);
    }
    ...
    void updateSelectedNodesMarking (final boolean markIt)
    {
        ...
        {
            final AbstractInfoNode<?>    selNode=
                    (AbstractInfoNode<?>) ((selPath == null) ? null : selPath.getLastPathComponent());
            markNode(model, selNode, markIt);

            // make sure that de-selection of one child de-selects its parent
            if (!markIt)
                deselectTillRoot(model, selNode);
        }
    }
    ...
    private void deselectTillRoot (final DefaultTreeModel model, final AbstractInfoNode<?>    selNode)
    {
        ...
        if ((parent == null) || (!(parent instanceof AbstractInfoNode<?>)))
            return;
    ...
    {
        if (selNode.isSelected() == markIt)
            return false;

        selNode.setSelected(markIt);
        /*
         *  NOTE: we are issuing a node-structure-changed event even
         *  though we only changed this node since the change might
         *  propagate to its sub-ordinate nodes
         */
...
//                _pkgsTree.setCellEditor(new NodeCellEditor(_pkgsTree, getResourcesAnchor()));
            }
            else if ("aspect-target".equalsIgnoreCase(name))
            {
                if (_targetAutoComplete != null)
                    throw new IllegalStateException("Element=" + name + " re-specified");
    ...
    public void layoutComponent () throws RuntimeException
    {
        ...
        if (_pkgsTree != null)
        {
            if (_contextMenu != null)
                _pkgsTree.addMouseListener(new MouseAdapter() {
                        /*
    ...
    /*
     * @see net.community.chest.javaagent.dumper.ui.AspectGeneratorHandler#handleProcessedNode(net.community.chest.javaagent.dumper.ui.tree.AbstractInfoNode)
     */
    @Override
    ...
    /*
     * @see net.community.chest.javaagent.dumper.ui.AspectGeneratorHandler#doneGenerating(net.community.chest.javaagent.dumper.ui.AspectGeneratorThread)
     */
    @Override
    ...
    protected void generateAspects ()
    {
        ...
        final File        outputFolder=
            ((outputPath == null) || (outputPath.length() <= 0)) ? null : new File(outputPath);
        ...
        if (outputFolder.exists())
        {
            final int    nRes=JOptionPane.showConfirmDialog(
                this, "Target folder exists and will be emptied - continue ?", "Confirm target folder removal", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    /*
     * @see net.community.chest.javaagent.dumper.ui.SelectiblePackageInfoHandler#doneLoadingDumperData(net.community.chest.javaagent.dumper.ui.DumperDataLoaderThread)
     */
    @Override
    ...
    /*
     * @see net.community.chest.javaagent.dumper.ui.SelectiblePackageInfoHandler#processSelectiblePackageInfo(net.community.chest.javaagent.dumper.ui.data.SelectiblePackageInfo)
     */
    @Override
    ...
    private void processMainArguments (final String ... args)
    {
        ...
        for (int    aIndex=0; aIndex < numArgs; aIndex++)
        {
            ...
            if (optName.startsWith("--load"))
                loadFile(new File(extractOptionValue(optName)), LOAD_CMD, null);
            else if (optName.startsWith("--save"))
                saveFile(new File(extractOptionValue(optName)), null);
        }
    }
