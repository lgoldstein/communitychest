...
import net.community.chest.io.output.WriterEmbedder;

/**
 * <P>Copyright 2007 as per GPLv2</P>
 *
 * @author Lyor G.
 * @since Sep 19, 2007 9:13:16 AM
 */
public class EOMHunterWriter extends WriterEmbedder implements EOMDataStreamHunter {
    /**
     * If set, then EOM detected
     */
    ...
    /**
     * @param w writer to write data to while "hunting" for EOM
     ...
     */
    public EOMHunterWriter (Writer w, boolean echoEOM, boolean realClose)
    {
        super(w, realClose);
        setEchoEOM(echoEOM);
    }
    /**
     * Creates a writer that does not echo the EOM (if found), and calls
     * the underlying writer's "close" method on calling its "close".
     * @param w writer to write data to while "hunting" for EOM
     */
    public EOMHunterWriter (Writer w)
    {
        this(w, false, true);
    }
    // maximum EOM signal is CRLF.CRLF (we accept LF.LF as well as similar variations)
    private final char[]    _lastBuf=new char[EOMHunter.MAX_EOM_SEQLEN];
     ...
     * but the data is not written to the real writer. <B>Note:</B>
     * this method can be used to set the internal EOM state to some
     * initial value without affecting the real writer
     * @throws IOException unable to write
     */
    protected void write (char[] buf, int offset, int len, boolean echoToOutput) throws IOException
    {
        if (null == this.out)
            throw new IOException("No writer to write buffer to");
        if ((offset < 0) || (len < 0))
    ...
    {
        if (null == this.out)
            throw new IOException("No writer to write character to");
        ...
        else    // character might complete a sequence
        {
            ...
            if (_bufPos >= _lastBuf.length)
            {
                ...
                _lastBuf[_lastBuf.length-1] = (char) val;
            }
            else    // append new value
            {
                _lastBuf[_bufPos] = (char) val;
                _bufPos++;
            }
    ...
    {
        if ((null == s) || (offset < 0) || (len < 0) || ((offset+len) > s.length()))
            throw new IOException("Bad/Illegal string to write");

        super.write(s, offset, len);
    }
    /* NOTE !!! must be called to ensure echoing of EOM and/or flushing of internal EOM buffer
     * @see net.community.chest.io.WriterEmbedder#close()
     */
    @Override
