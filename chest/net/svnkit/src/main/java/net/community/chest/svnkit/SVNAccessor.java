...
public class SVNAccessor {
    ...
    {
        if ((null == _opts) && createIfNotExist)
            _opts = SVNWCUtil.createDefaultOptions(getConfigDirectory(), isReadOnlyAccess());
        return _opts;
    }
    ...
    {
        if ((null == _authMgr) && createIfNotExist)
            _authMgr = SVNWCUtil.createDefaultAuthenticationManager(getConfigDirectory(), getUsername(), getPassword(), isStoreAuth());
        return _authMgr;
    }
    ...
    {
        if ((null == _mgr) && createIfNotExist)
            _mgr = SVNClientManager.newInstance(opts, mgr);
        return _mgr;
    }
