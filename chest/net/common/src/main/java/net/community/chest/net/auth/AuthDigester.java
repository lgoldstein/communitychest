...
public abstract class AuthDigester {
	...
	public static final byte[] getAuthStringBytes (String s)
	{
		...
		if (sLen <= 0)
			throw new IllegalArgumentException("Null/empty authentication string");
		...
		for (int	sIndex=0; sIndex < sLen; sIndex++)
		{
			...
			if ((sch < 0) || (sch > 255))
				throw new IllegalStateException("Authentication string character at position=" + sIndex + " not in 0-255 range (value=" + s + ")");
	...
	{
		final int	textLen=(null == text) /* should not happen */ ? 0 : text.length(), 			
					keyLen=(null == key) /* should not happen */ ? 0 : key.length();
		if ((textLen <= 0) || (keyLen <= 0) || (null == md5Digester))
			throw new IllegalArgumentException("Null/empty text/key/digester");
		...
		if (keyLen > MAX_MD5_PAD_LEN)
			keyBytes = md5Digester.digest(keyBytes);
		...
		// perform inner digest
		{
			...
			for (int	cIndex=0; cIndex < textLen; cIndex++)
			{
				...
				if ((cch < 0) || (cch > 255))
					throw new IllegalStateException("Text char at position=" + cIndex + " is not in 0-255 range");
				chlngBytes[cIndex] = (byte) cch;
			}
			md5Digester.update(chlngBytes);
		}

		// perform outer digest + return result
		{
	 ...
	 */
	public static final String getCRAMMD5ChallengeBaseResponse (String chlng, String username, String password, MessageDigest md5Digester)
		throws IllegalArgumentException
	{
		if ((null == username) || (username.length() <= 0))
			throw new IllegalArgumentException("Null/Empty username supplied");
	 ...
	 */
	public String getCRAMMD5ChallengeBaseResponse (String chlng, String username, String password)
	 ...
	 * @throws IllegalArgumentException if null/empty challenge/username/password/digester
	 */
	public static final String getCRAMMD5ChallengeResponse (String chlng, String username, String password, MessageDigest md5Digester)
		throws IllegalArgumentException
	{
	 ...
	 * @throws IllegalArgumentException if null/empty challenge/username/password/digester
	 */
	public String getCRAMMD5ChallengeResponse (String chlng, String username, String password)
		throws NoSuchAlgorithmException, IllegalArgumentException
	{
	...
	{
        final int		uLen=(null == username) ? 0 : username.length(),
        				pLen=(null == password) ? 0 : password.length();
        if ((uLen <= 0) || (pLen <= 0))
        	throw new StreamCorruptedException(ClassUtil.getArgumentsExceptionLocation(AuthDigester.class, "appendPlainChallengeBaseResponse", username, password) + " null/empty arguments");
		if (null == sb)
			throw new IOException(ClassUtil.getArgumentsExceptionLocation(AuthDigester.class, "appendPlainChallengeBaseResponse", username, password) + " no " + Appendable.class.getName() + " instance supplied");
	 ...
	 */
	public static final String getPlainChallengeBaseResponse (String username, String password)
		throws IllegalStateException
	{
        final int	uLen=(null == username) ? 0 : username.length(),
					pLen=(null == password) ? 0 : password.length(),
        			aLen=Math.max(uLen,0) + 1 + Math.max(pLen,0) + 1;
        try
        {
        	final StringBuilder	sb=
        			((uLen <= 0) || (pLen <= 0)) ? null : new StringBuilder(aLen),
        						res=appendPlainChallengeBaseResponse(sb, username, password);
        	return res.toString();
        }
        catch(IOException e)
        {
	 ...
	 * @throws IllegalStateException if unable to build response
	 */
	public static final String getPlainChallengeResponse (String username, String password)
