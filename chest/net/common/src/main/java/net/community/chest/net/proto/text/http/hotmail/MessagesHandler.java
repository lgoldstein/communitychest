...
public final class MessagesHandler {
    ...
    public static final String     HotmailMsgsNamespace="urn:schemas:mailheader:",
                                EntityTypePropName="isfolder",
                                ContentLengthPropName="getcontentlength",
                                SenderPropName="from",
                                SubjectPropName="subject",
                                RcvDatePropName="date",
                                RfcIdPropName="message-id";

    /**
     * XML payload used to retrieve message envelope information
     */
    public static final String envInfoXml=
        "<?xml version=\"1.0\"?>\r\n"
            + "<D:propfind xmlns:D=\"DAV:\" xmlns:hm=\"" + HotmailProtocol.HOTMAIL_XML_NAMESPACE
            + "\" xmlns:m=\"" + HotmailMsgsNamespace + "\">\r\n"
                + "\t<D:prop>\r\n"
     ...
     */
    public static final String quickInfoXml=
        "<?xml version=\"1.0\"?>\r\n"
            + "<D:propfind xmlns:D=\"DAV:\" xmlns:hm=\"" + HotmailProtocol.HOTMAIL_XML_NAMESPACE
            + "\" xmlns:m=\"" + HotmailMsgsNamespace + "\">\r\n"
                + "\t<D:prop>\r\n"
    ...
    public static class MsgInfo {
        ...
        private Calendar    _rcvDate    /* =null */;
        public MsgInfo ()
        {
            super();
        }
    ...
    public static class MsgEnvelope extends MsgInfo {
        private String _sender    /* =null */, _subject    /* =null */, _rfcId    /* =null */;
        public MsgEnvelope ()
        {
            super();
        }
     ...
     */
    public static abstract class AbstractMessagePropHandler implements MessagePropHandler {
        ...
        /*
         * @see net.community.chest.net.proto.text.http.hotmail.MessagesHandler.MessagePropHandler#updateEnvelope(org.w3c.dom.Node, net.community.chest.net.proto.text.http.hotmail.MessagesHandler.MsgEnvelope)
         */
        @Override
        public boolean updateEnvelope (final Node infoNode, final MsgEnvelope msgEnv)
        {
            ...
            if ((nodeVal != null) && (nodeVal.length() > 0))
                return updateEnvelope(nodeVal, msgEnv);
    ...
    public static final class MessageHRefPropHandler extends AbstractMessagePropHandler {
        ...
        /*
         * @see net.community.chest.net.proto.text.http.hotmail.MessagesHandler.AbstractMessagePropHandler#updateEnvelope(java.lang.String, net.community.chest.net.proto.text.http.hotmail.MessagesHandler.MsgEnvelope)
         */
        @Override
    ...
    public static final class MessageRcvDatePropHandler extends AbstractMessagePropHandler {
        ...
        public static final int decodeDateTimeComponent (final String    dtVal,
                                                         final int        startPos,
                                                         final int        numDigits,
                                                         final char        delim,
                                                         final int        minVal,
                                                                ...
                                                                throws NumberFormatException
        {
            ...
            if ((startPos < 0) || (numDigits <= 0) || (dtvLen < endPos))
                throw new NumberFormatException("Bad/Illegal position parameters");

            if (delim != '\0')
            {
                if ((dtvLen < (endPos+1)) || (dtVal.charAt(endPos) != delim))
                    throw new NumberFormatException("Bad/Illegal delimiter parameter");
            }

            final String    numVal=dtVal.substring(startPos, endPos);
            final int        cVal=Integer.parseInt(numVal);
            if ((cVal < minVal) || (cVal > maxVal))
                throw new NumberFormatException("Retrieved value (" + cVal + " is out of range " + minVal + "-" + maxVal);
        ...
        {
            if ((null == dtVal) || (dtVal.length() <= 0) || (null == rcvDate))
                return false;

            try
            {
                final int    year=decodeDateTimeComponent(dtVal, 0, 4, '-', 1900, 9999),
                            month=decodeDateTimeComponent(dtVal, 5, 2, '-', 1, 12),
                            day=decodeDateTimeComponent(dtVal, 8, 2, 'T', 1, 31),
                            hour=decodeDateTimeComponent(dtVal, 11, 2, DateUtil.DEFAULT_TMSEP, 0, 23),
                            minute=decodeDateTimeComponent(dtVal, 14, 2, DateUtil.DEFAULT_TMSEP, 0, 59),
                            second=decodeDateTimeComponent(dtVal, 17, 2, '\0', 0, 59);
        ...
        public static final Calendar decodeMessageRcvDate (final String dtVal) throws IllegalArgumentException
        {
            ...
            rcvDate.set(Calendar.ERA, GregorianCalendar.AD);    // just making sure

            if (!decodeMessageRcvDate(dtVal, rcvDate))
                throw new IllegalArgumentException("Bad/Illegal string value: " + dtVal);

            // this actually forces a re-calculation of the internal milliseconds value
            if (rcvDate.get(Calendar.ERA) != GregorianCalendar.AD)
                throw new IllegalStateException("ERA value mismatch: value=" + rcvDate.get(Calendar.ERA) + " expected=" + GregorianCalendar.AD);
     ...
     */
    public static abstract class AbstractNumMessagePropHandler extends AbstractMessagePropHandler {
    ...
    public static final class MessageContentLengthPropHandler extends AbstractNumMessagePropHandler {
        ...
        /*
         * @see net.community.chest.net.proto.text.http.hotmail.MessagesHandler.ANumMessagePropHandler#updateEnvelope(int, net.community.chest.net.proto.text.http.hotmail.MessagesHandler.MsgEnvelope)
         */
        @Override
    ...
    public static final class MessageEntityTypePropHandler extends AbstractNumMessagePropHandler {
        ...
        /*
         * @see net.community.chest.net.proto.text.http.hotmail.MessagesHandler.AbstractNumMessagePropHandler#updateEnvelope(int, net.community.chest.net.proto.text.http.hotmail.MessagesHandler.MsgEnvelope)
         */
        @Override
        protected boolean updateEnvelope (final int nodeVal, final MsgEnvelope msgEnv)
        {
    ...
    static {
        final Object[] msgProps={
            "href",                    new MessageHRefPropHandler(),
            EntityTypePropName,     new MessageEntityTypePropHandler(),
            ContentLengthPropName,    new MessageContentLengthPropHandler(),
            SenderPropName,            new MessageSenderPropHandler(),
            SubjectPropName,        new MessageSubjectPropHandler(),
            RcvDatePropName,        new MessageRcvDatePropHandler(),
            RfcIdPropName,            new MessageRfcIdPropHandler()
        };
        for (int    i=0; i < msgProps.length; i+= 2)
            msgPropHandlersMap.put((String) msgProps[i], (MessagePropHandler) msgProps[i+1]);
    }
    /**
    ...
    public static final MsgEnvelope updateMsgEnvelope (final NodeList msgNodes, final MsgEnvelope msgEnv)
    {
        ...
        for (int    nIndex=0; nIndex < numNodes; nIndex++)
        {
            ...
            if (handler != null)
            {
                if (!handler.updateEnvelope(nd, msgEnv))
                    return null;
            }
            else if (null == updateMsgEnvelope(nd.getChildNodes(), msgEnv))
                return null;
        }
    ...
    public static final MsgEnvelope extractMsgEnvelope (final Node nd)
    {
        ...
        // must have at least message reference
        if ((null == msgEnv) || (null == msgEnv.getHRef()) || (msgEnv.getHRef().length() <= 0))
            return null;
    ...
    public static final boolean extractMessagesInfo (final NodeList docNodes, final Collection<MsgEnvelope> msgs)
    {
        ...
        for (int    nIndex=0; nIndex < numNodes; nIndex++)
        {
            ...
            if ((nodeName != null) && (nodeName.length() > 0) && nodeName.equalsIgnoreCase("response"))
            {
                ...
                if (msgEnv != null)    // should not happen
                    msgs.add(msgEnv);
                continue;
            }
            ...
            if (nd.hasChildNodes())
            {
                if (!extractMessagesInfo(nd.getChildNodes(), msgs))
                    return false;
            }
        }
    ...
    public static final Collection<MsgEnvelope> extractMessagesInfo (final Document rspDoc) throws IllegalStateException
    {
        ...
        if (!extractMessagesInfo((null == rspDoc) ? null : rspDoc.getChildNodes(), msgs))
            throw new IllegalStateException("Cannot extract messages information");
