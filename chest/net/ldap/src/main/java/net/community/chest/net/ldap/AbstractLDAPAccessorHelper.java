...
public abstract class AbstractLDAPAccessorHelper extends AbstractLDAPAccessor {
    ...
    // CAVEAT EMPTOR when changing them
    ...
    private DirContext    _context    /* =null */;
    ...
    {
        if (getContext() != null)
            throw new ContextNotEmptyException("updateEnvironmentIntParam(" + propName + ")[" + propVal + "] context already initialized");

        try
        {
            if ((null == propName) || (propName.length() <= 0)
             || (null == propVal) || (propVal.length() <= 0))
                throw new NumberFormatException("Missing property name/value");
    ...
    /*
     * @see net.community.chest.net.ldap.LDAPAccessor#setProtocolVersion(int)
     */
    @Override
    public void setProtocolVersion (int ver) throws NamingException
    {
        if (ver <= 0)
            throw new InvalidAttributeValueException("setProtocolVersion(" + ver + ") invalid value");

        updateEnvironmentIntParam("java.naming.ldap.version", String.valueOf(ver));
    }
    /*
     * @see net.community.chest.net.ldap.LDAPAccessor#bind(java.lang.String, int, java.lang.String, java.lang.String)
     */
    @Override
    public void bind (String host, int port, String bindDN, String bindPassword) throws NamingException
    {
        if (_context != null)
            throw new ContextNotEmptyException("bind(" + host + ")[" + port + "] - context already activated");

        if ((null == host) || (host.length() <= 0)
         || (port <= 0) || (port >= 0x0000FFFF))
            throw new InvalidAttributeValueException("bind(" + host + ")[" + port + "] invalid network location");
        ...
        {
            if ((null == bindPassword) || (bindPassword.length() <= 0))
                throw new InvalidAttributeValueException("bind(" + host + ")[" + port + "] no bind password provided");
