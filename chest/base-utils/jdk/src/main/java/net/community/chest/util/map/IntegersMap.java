...
public class IntegersMap<V> extends NumbersMap<Integer,V> implements PubliclyCloneable<IntegersMap<V>> {
    ...
    public static class IntegerEntriesComparator extends NumberEntriesComparator {
        ...
        public static final IntegerEntriesComparator    DEFAULT=
            new IntegerEntriesComparator();
    }
    /*
    ...
    public Set<Entry<Integer, V>> entrySet (final int fromKey, final int toKey)
    {
        ...
        for (int    eIndex=0; eIndex < numItems; eIndex++)
        {
            ...
            if (k >= fromKey)
            {
                ...
                if (null == ts)
                    ts = new TreeSet<Entry<Integer,V>>(getEntryComparator());
                ts.add(e);
            }
        }
    ...
    public V remove (final int key) throws IllegalArgumentException, IllegalStateException
    {
        ...
        if ((null == prev) || (Integer.MAX_VALUE == k[eIndex]))
    ...
    public Set<Integer> keySet (final int fromKey, final int toKey)
    {
        ...
        for (final Entry<Integer,V> e : eSet)
        {
            final Integer    k=(null == e) /* should not happen */ ? null : e.getKey();
            if (null == k)    // should not happen
                continue;

            if (null == ret)
                ret = new TreeSet<Integer>(comparator());
            ret.add(k);
        }
    ...
    public Integer firstKey (final int fromKey, final int toKey) throws NoSuchElementException
    {
        ...
        for (final int k : ks)
        {
            ...
            if (k >= fromKey)
            {
                if (Integer.MAX_VALUE == k)    // should not happen
    ...
    public SortedMap<Integer, V> subMap (final Integer fromKey, final Integer toKey)
    {
        ...
        for (int    eIndex=0; eIndex < numItems; eIndex++)
        {
            ...
            if (null == res)
                res = new IntegersMap<V>(getValuesClass(), numItems, getGrowSize());
    ...
    public static final Integer    MIN_KEY=Integer.valueOf(Integer.MIN_VALUE),
                                MAX_KEY=Integer.valueOf(Integer.MAX_VALUE - 1),
                                TAIL_KEY=Integer.valueOf(MAX_KEY.intValue() + 1);
    /*
    ...
    public IntegersMap<V> clone () throws CloneNotSupportedException
    {
        ...
        final IntegersMap<V>    cm=(IntegersMap<V>) super.clone();
