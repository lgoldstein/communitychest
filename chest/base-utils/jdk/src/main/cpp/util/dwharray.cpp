...
size_t formatted_string_to_dword_array (const char		str_in[],
													 ...
													 EXC_PARAM(exc))
{
	...
	for ( ; (len < out_len) && (sdx < str_len); bp++, len++)
	{
		if (delim != '\0')
		{
			...
			tsp = (strp + MAX_DWORD_HEX_DISPLAY_LENGTH);

		tLen = (size_t) (tsp-strp);
		if ((sdx + tLen) > str_len)
			break;

		*bp = isHex ? hex_argument_to_dword(strp, (size_t) (tsp-strp), exc) : argument_to_dword(strp, (size_t) (tsp-strp), exc);
		if (EXC_LVAL(exc) != EOK)
			return(len);
...
size_t hex_string_to_dword_array (const char		str_in[],
											 const size_t	str_len,
											 DWORD			out_array[],
											 const size_t	out_len,
											 const char    delim,
...
size_t string_to_dword_array (const char		str_in[],
										const size_t	str_len,
										DWORD				out_array[],
										const size_t	out_len,
										const char		delim,
