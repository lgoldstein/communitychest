...
public class MethodsComparator extends AbstractComparator<Method> {
    ...
    {
        if (Modifier.isStatic(m1))
            return Modifier.isStatic(m2) ? 0 : (+1);
        else
            return Modifier.isStatic(m2) ? (-1) : 0;
    }
    ...
    {
        // null object(s) come first
        if (null == m1)
            return (null == m2) ? 0 : (-1);
        else if (null == m2)
            return (+1);

        // make sure methods belong to compatible classes
        final Class<?>    c1=m1.getDeclaringClass(), c2=m2.getDeclaringClass();
        if ((!c1.isAssignableFrom(c2)) && (!c2.isAssignableFrom(c1)))
            throw new ClassCastException("compareMethods(" + m1.getName() + "," + m2.getName() + ") not of compatible classes: " + c1.getName() + " <> " + c2.getName());

        // lexicographical order takes precedence
        {
        ...
        // methods have same name - check parameters
        {
        ...
        // methods are virtually the same - check visibility
        {
        ...
        // prefer a "stronger" implementor first
        {
     ...
     * @throws ClassCastException if objects are not methods or not from
     * compatible classes
     */
    @Override
     ...
     */
    public static final MethodsComparator    ASCENDING=new MethodsComparator(true),
                                            DESCENDING=new MethodsComparator(false);
}
