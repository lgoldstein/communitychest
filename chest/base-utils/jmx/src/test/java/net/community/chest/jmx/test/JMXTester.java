...
public class JMXTester extends TestBase implements JMXTesterMBean, MBeanRegistration {
	...
	/*
	 * @see net.community.chest.test.jmx.JMXTesterMBean#doVoidOperation()
	 */
	@Override
	...
	/*
	 * @see net.community.chest.test.jmx.JMXTesterMBean#getCurrentTime()
	 */
	@Override
	...
	/*
	 * @see net.community.chest.test.jmx.JMXTesterMBean#compareCurrentTime(long)
	 */
	@Override
	public long compareCurrentTime (long tStamp)
	{
		final long	curTime=getCurrentTime(),
					diffTime=tStamp - curTime;
	...
	/*
	 * @see net.community.chest.test.jmx.JMXTesterMBean#getStringValue()
	 */
	@Override
	...
	/*
	 * @see net.community.chest.test.jmx.JMXTesterMBean#setStringValue(java.lang.String)
	 */
	@Override
	...
	/*
	 * @see net.community.chest.test.jmx.JMXTesterMBean#toLowerCase(java.lang.String)
	 */
	@Override
	...
	private static final int testMBean (final PrintStream			out,
										final BufferedReader		in,
										final MBeanServerConnection	s,
								...
								for (final MBeanOperationInfo o : opers)
								{
									...
									if ((params != null) && (params.length > 0))
									{
										for (final MBeanParameterInfo p : params)
											out.println("\t\t" + p.getName() + "[" + p.getType() + "]: " + p.getDescription());
									}
								}
							}
							else
					...
					case 'M'	:
						{
							...
							for ( ; ; )
							{
								...
								if ((null == attrName) || (attrName.length() <= 0) || isQuit(attrName))
									break;
								...
								switch(aType)
								{
									case 'S'	: aValue = attrValue; break;
									case 'I'	: aValue = Integer.valueOf(attrValue); break;
									case 'L'	: aValue = Long.valueOf(attrValue); break;
									case 'B'	: aValue = Boolean.valueOf(attrValue); break;
									default		: aValue = null;
								}
	...
	private static final void testXMBeansAdd (final PrintStream out, final BufferedReader in, final MBeanServerConnection s)
	{
		try
		{
			final Map<ManagementFactoryBeanType,MBeanRegistrationResult>	mbm=
				ManagementFactoryBeanType.registerAllMBeans((MBeanServer) s);
			final Collection<? extends Map.Entry<ManagementFactoryBeanType,MBeanRegistrationResult>>	mel=
				((null == mbm) || (mbm.size() <= 0)) ? null : mbm.entrySet();
	...
	private static final int testMBeansAdd (final PrintStream out, final BufferedReader in, final MBeanServerConnection s)
	{
		...
		{
			if ((null == (path=getval(out, in, "(F)actory/XML file path/(Q)uit)"))) || (path.length() <= 0))
				continue;
			...
			try
			{
				...
				if ((errs != null) && (errs.size() > 0))
				{
					for (final ElementIndicatorExceptionContainer ind : errs)
					{
						...
						System.err.println("\tupdateDefaultMBeans(" + elem.getTagName()
									+ "[" + MBeanEntryDescriptor.NAME_ATTR + "="
									+  elem.getAttribute(MBeanEntryDescriptor.NAME_ATTR)
			...
			for (final MBeanEntryDescriptor d : descs)
			{
				...
				try
				{
					...
					if (null == inst)
						throw new NoSuchElementException("No object instance created");
					...
					case 'F'	:
						{
							...
							{
								if ((null == (name=getval(out, in, "MBean name (or Quit)"))) || (name.length() <= 0))
									continue;
								...
								try
								{
									...
									if (null == mbi)
										throw new InstanceNotFoundException("No MBeanInfo instance found");
	...
	public static final int testMBeanServer (final PrintStream out, final BufferedReader in, final String ... args)
	{
		...
		{
			final String	ans=
				(aIndex < numArgs) ? args[aIndex] : getval(out, in, "[S]ervers/(C)reate/(F)ind/(R)emote/(Q)uit");
			if (isQuit(ans))
				break;
					...
					case 'C'	:
						{
							...
							final MBeanServer	s=((null == dmName) || (dmName.length() <= 0))
										? MBeanServerFactory.createMBeanServer()
										: MBeanServerFactory.createMBeanServer(dmName)
										;
							if (s != null)
								testMBeanServer(out, in, s);
							else
								throw new UnsupportedOperationException("No server created");
						}
						break;
					...
					case 'S'	:
						{
							final String							agentId=
										('S' == op) ? null : getval(out, in, "agent ID (ENTER=all)");
							final Collection<? extends MBeanServer>	servers=
										MBeanServerFactory.findMBeanServer(
											((null == agentId) || (agentId.length() <= 0)) ? null : agentId);
							final int								numServers=
										(null == servers) ? 0 : servers.size();
							if (numServers > 0)
							{
								for (final MBeanServer s : servers)
									testMBeanServer(out, in, s);
							}
							else
