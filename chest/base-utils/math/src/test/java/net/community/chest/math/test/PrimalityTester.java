...
public class PrimalityTester extends Assert {
	...
	{
		if (value < 0L)	// deal only with positive numbers
			return findFirstFactor(0L - value);
		if (value <= 1L)
			return null;
	...
	public void testPrimeBitPositions ()
	{
		generatePrimes(3L, Integer.MAX_VALUE, new PrimeHandler() {
			/*
			 * @see net.community.chest.math.test.PrimalityTest.PrimeHandler#handlePrime(long)
			 */
			@Override
			public boolean handlePrime (final long number)
			{
				...
				final String	posValue=String.valueOf(bitPos),
								revValue=String.valueOf(CollectionsUtils.reverse(bitPos));
				System.out.append("\t")
	...
	protected long generatePrimes (final long startPos, final long endPos, final PrimeHandler handler)
	{
		...
		for (long curValue=Math.max(2L, startPos); (curValue > 0L) && (curValue < endPos); curValue++)
		{
			...
			numPrimes++;
			if (!handler.handlePrime(curValue))
				break;
		}

		final long	endTime=System.nanoTime(), duration=endTime - startTime;
		System.out.append("Processed ")
		          .append(String.valueOf(numPrimes))
		          .append(" primes in ")
