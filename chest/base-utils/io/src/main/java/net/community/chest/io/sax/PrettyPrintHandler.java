...
public class PrettyPrintHandler extends DefaultHandler2 {
    ...
    protected CharSequence prepareTabs (Boolean decreaseTabs) throws SAXException
    {
        ...
        {
            // TODO take into account TAB length
            if (decreaseTabs.booleanValue())
            {
                final int    len=_tabsBuilder.length();
                if (len <= 0)
                    throw new SAXException("Mismatched tabs state");
    ...
    protected Appendable prepareOutput (boolean startNewLine, Boolean decreaseTabs)
        throws IOException, SAXException
    {
        final Appendable    a=getOutput();
        if (null == a)
            throw new SAXException("No " + Appendable.class.getSimpleName() + " output provided");
    ...
    private final Stack<Triplet<String,Boolean,Boolean>>    _elemsStack=
        new Stack<Triplet<String,Boolean,Boolean>>();
    /*
    ...
    public void startDocument () throws SAXException
    {
        ...
        if (remElems > 0)
            throw new SAXException("Pre-populated " + remElems + " elements");
    }
    /*
    ...
    public void comment (char[] ch, int start, int length) throws SAXException
    {
        ...
        try
        {
            ...
            a.append(DOMUtils.XML_COMMENT_START)
            ...
            throws SAXException
    {
        if (length > 0)
        {
            ...
            final Triplet<String,Boolean,Boolean>    elemState=
                ((null == _elemsStack) || _elemsStack.isEmpty()) ? null : _elemsStack.peek();
            final Boolean                            prevWrite=
                (null == elemState) ? null : elemState.getV3();
            if (null == prevWrite)
                throw new SAXException("characters(" + cb + ") no previous element state");
        ...
        throws SAXException
    {
        ...
        // prevent inline closure of previous element
        final Triplet<String,Boolean,Boolean>    prevState=
            ((null == _elemsStack) || _elemsStack.isEmpty()) ? null : _elemsStack.peek();
        final Boolean                            prevSubClassed=
            (null == prevState) ? null : prevState.getV2();
        ...
        try
        {
            ...
            if ((prevSubClassed != null) && (!prevSubClassed.booleanValue()))
            {
                final Appendable    a=getOutput();
                a.append(DOMUtils.XML_ELEM_END_DELIM);
                prevState.setV2(Boolean.TRUE);    // mark fact that sub-classed
                prevState.setV3(Boolean.TRUE);    // mark fact that closed the tag
            }
            ...
            {
                final String    aName=attributes.getLocalName(aIndex),
                                aValue=attributes.getValue(aIndex);
    ...
    {
        final Triplet<String,Boolean,Boolean>    elemState=
            ((null == _elemsStack) || _elemsStack.isEmpty()) ? null : _elemsStack.pop();
        final Boolean                            subClassed=
            (null == elemState) ? null : elemState.getV2(),
                                                withText=
            (null == elemState) ? null : elemState.getV3();

        if ((null == withText) || (null == subClassed))
            throw new SAXException("endElement(" + qName + ") no previous element state");

        final String    elemName=elemState.getV1();
        if (StringUtil.compareDataStrings(qName, elemName, true) != 0)
            throw new SAXException("endElement(" + qName + ") mismatched name - expected=" + elemName);

        try
        {
            final Appendable    a=prepareOutput(subClassed.booleanValue(), null);
            if (subClassed.booleanValue())
                appendTabs(a);

            if (withText.booleanValue() || subClassed.booleanValue())
            {
                a.append(DOMUtils.XML_TAG_CLOSURE_SEQ)
                 .append(qName)
                 .append(DOMUtils.XML_ELEM_END_DELIM)
                 ;
            }
            else
                a.append(DOMUtils.XML_TAG_INLINE_SEQ);
    ...
    public void endDocument () throws SAXException
    {
        ...
        if (tabsLen > 0)
            throw new SAXException("Remainder of " + tabsLen + " TABS");

        final int    remElems=(null == _elemsStack) ? 0 : _elemsStack.size();
        if (remElems > 0)
            throw new SAXException("Remainder of " + remElems + " elements");
