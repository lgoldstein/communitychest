...
public final class Hex {
    ...
    public static final int getHexSequenceLength (final CharSequence cs, final int offset, final int len, final char sep)
    {
        ...
        if ((offset < 0) || (len < 0) || (maxPos > csLen))
        ...
        {
            // last place has no separator - so we compensate for it
            if (((len+1) % 3) != 0)
                return (0 - len - 1);
        }
        ...
        {
            final char    hi=cs.charAt(hPos),
                        lo=(lPos < maxPos) ? cs.charAt(lPos) : '*'    /* bad char by design */;
            if (!isHexDigit(hi))
                return (0 - hPos - 1);
            if (!isHexDigit(lo))
                return (0 - lPos - 1);
            ...
            if (sep != '\0')
            {
                // last position does not contain the separator
                if (hPos >= maxPos)
                    break;
    ...
    {
        if (null == sb)
            throw new IOException(ClassUtil.getArgumentsExceptionLocation(Hex.class, "appendHex", Byte.valueOf(b), Boolean.valueOf(useUppercase)) + " no " + Appendable.class.getName() + " instance");

        final char[]    hexDigits=useUppercase ? upperHex : lowerHex,
                        valChars={ hexDigits[(b >> 4) & 0x0F], hexDigits[b & 0x0F] };
     ...
     */
    public static final String toString (byte[] b, int startOffset, int len, char delim, boolean useUppercase)
        throws IllegalArgumentException, IllegalStateException
    {
        if ((null == b) || (startOffset < 0) || (len <= 0) || ((startOffset+len) > b.length))
            throw new IllegalArgumentException("Bad/Illegal input buffer");

        try
        {
            final StringBuilder    sb=
                appendHex(new StringBuilder((len + 1 /* avoid last char issues */) * 2), b, startOffset, len, delim, useUppercase);
            return sb.toString();
        }
        catch(IOException e)
        {
     ...
     */
    public static final String toString (byte[] b, char delim, boolean useUppercase)
        throws IllegalArgumentException, IllegalStateException
    {
    ...
    // returns null if null value provided
    public static final String toString (Byte b, boolean useUpperCase)
    {
    ...
    {
        final int    hiPosUpper=upperHexDigitsString.indexOf(hiChar),
                    hiPos=(hiPosUpper < 0) ? lowerHexDigitsString.indexOf(hiChar) : hiPosUpper,
                    loPosUpper=upperHexDigitsString.indexOf(loChar),
                    loPos=(loPosUpper < 0) ? lowerHexDigitsString.indexOf(loChar) : loPosUpper;

        if ((hiPos < 0) || (loPos < 0))
            throw new NumberFormatException("Not HEX characters");
    ...
    {
        if ((len < 2) || (offset < 0))
            throw new NumberFormatException("fromString(" + cs + ")[" + offset + "/" + len + "] bad/Illegal input to convert to HEX");
    ...
    public static final byte[] toByteArray (final CharSequence cs, final int offset, final int len, final char delim) throws NumberFormatException
    {
        ...
        if ((offset < 0) || (len <= 0))
            throw new NumberFormatException("toByteArray(" + cs + ")[" + offset + "-" + maxPos + "] Illegal range to decode");
        if (0 == len)
            return null;

        if ((null == cs) || ((maxPos > cs.length())))
            throw new NumberFormatException("toByteArray(" + cs + ")[" + offset + "-" + maxPos + "] Illegal data values to decode");

        final int    seqLen=getHexSequenceLength(cs, offset, len, delim);
        if (seqLen < 0)
            throw new NumberFormatException("toByteArray(" + cs + ")[" + offset + "-" + maxPos + "] Invalid HEX sequence at position=" + (0 - seqLen - 1));
        ...
        if (outOffset < out.length)    // should not happen
            throw new NumberFormatException("toByteArray(" + cs + ")[" + offset + "-" + maxPos + "] Incomplete HEX sequence");
    ...
    {
        if ((null == cs) || (offset < 0) || (len < MIN_HEX_PATTERN))
            return false;
        if ((offset + MIN_HEX_PATTERN) > cs.length())
            return false;

        final char    ch1=cs.charAt(offset), ch2=cs.charAt(offset + 1);
        if ((ch1 != '\\') || (!isHexModifier(ch2)))
            return false;
    ...
    // translates "\xHH" characters to their value(s) - ignores patterns that are not like this
    public static final String translateControlCharacters (final String s)
    {
        ...
        for ( ; (curPos >= lastPos) && (curPos < sLen); curPos = s.indexOf('\\', curPos))
        {
            ...
            if (null == sb)
                sb = new StringBuilder(sLen - MIN_HEX_PATTERN);
            // add the preceding data (if any)
            if (curPos > lastPos)
            {
            ...
            curPos += MIN_HEX_PATTERN;    // skip the pattern
            if ((lastPos=curPos) >= sLen)
                break;
        }

        if ((null == sb) || (sb.length() <= 0))
            return s;    // if nothing translated then return the original
