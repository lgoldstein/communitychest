...
public abstract class AbstractIOTransformer extends BaseIOTransformer {
    ...
    protected SAXParserFactory getSAXParserFactory ()
    {
        ...
        if ((null == facName) || (facName.length() <= 0))
        ...
        throws ParserConfigurationException, SAXException
    {
        ...
        if (null == fac)
            throw new ParserConfigurationException("No " + SAXParserFactory.class.getSimpleName() + " available");

        return fac.newSAXParser();
    }

    public abstract void transform (InputSource src, Appendable w)
    ...
    public void transform (StreamSource xmlSource, StreamResult outputTarget)
        throws TransformerException
    {
        final InputSource    src=
            TransformerUtil.resolveInputSource(xmlSource);
        final Closeable        tgt=
            TransformerUtil.resolveOutput(outputTarget, getURLOpenTimeout());
        final Appendable    a=
            (tgt instanceof Appendable) ? (Appendable) tgt : null;
        final OutputStream    o=
            ((null == a) && (tgt instanceof OutputStream)) ? (OutputStream) tgt : null;
        try
        {
            if (a != null)
                transform(src, a);
                ...
                throw new TransformerException("No output provided");
        }
        catch(IOException e)
        {
    ...
    public void transform (Source xmlSource, Result outputTarget)
            throws TransformerException
    {
        if ((null == xmlSource) || (!(xmlSource instanceof StreamSource)))
            throw new TransformerException("Null/non-" + StreamSource.class.getSimpleName() + " XML " + Source.class.getSimpleName());
