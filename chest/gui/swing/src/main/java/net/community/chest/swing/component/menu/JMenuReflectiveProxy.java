 ...
 *
 * @param <M> The {@link JMenu} generic type
 * @author Lyor G.
 * @since Mar 20, 2008 8:52:17 AM
 */
public class JMenuReflectiveProxy<M extends JMenu> extends JMenuItemReflectiveProxy<M> {
	public JMenuReflectiveProxy (Class<M> objClass) throws IllegalArgumentException
	{
		this(objClass, false);
	}

	protected JMenuReflectiveProxy (Class<M> objClass, boolean registerAsDefault)
	...
	/**
	 * Called by {@link #fromXmlChild(JMenu, Element)} when a sub-menu
	...
	public XmlValueInstantiator<? extends JMenuItem> getSubItemConverter (final Element elem) throws Exception
	{
		...
		if (isRadioButtonMenuItemElement(elem, itemClass))
			return JRadioButtonMenuItemReflectiveProxy.RADIOMENUITEM;
		else if (isCheckboxMenuItemElement(elem, itemClass))
			return JCheckBoxMenuItemReflectiveProxy.CBMENUITEM;
		else	// default
			return JMenuItemReflectiveProxy.MENUITEM;
	}
	/**
	 * Called by default implementation of {@link #createSubItem(JMenu, Element)}
	...
	/**
	 * Called by {@link #fromXmlChild(JMenu, Element)} when a menu-item
	 * XML {@link Element} is encountered in order to re-construct the
	 * sub-menu
	...
	/**
	 * Called by {@link #fromXmlChild(JMenu, Element)} when a separator
	 * XML {@link Element} is encountered in order to re-construct the
	 * sub-menu
	 * @param src The {@link JMenu} instance to which to add the sub item
	 * @param elem XML element to use for reconstructing the data
	...
	public static final JMenuReflectiveProxy<JMenu>	MENU=
					new JMenuReflectiveProxy<JMenu>(JMenu.class, true) {
				...
				@Override
				@CoVariantReturn
				public BaseMenu createInstance (Element elem) throws Exception
				{
					return (null == elem) ? null : new BaseMenu();
				}
		};
}
