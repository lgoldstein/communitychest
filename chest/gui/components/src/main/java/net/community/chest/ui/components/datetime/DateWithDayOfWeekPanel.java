...
public class DateWithDayOfWeekPanel extends PresetBorderLayoutPanel {
    ...
    protected DayOfMonthValuePanel createDayOfMonthPanel (Element elem, boolean useElement) throws RuntimeException
    {
        if ((null == elem) || (!useElement))
            return new DayOfMonthValuePanel();

        return new DayOfMonthValuePanel(elem);
    }

    private DayOfWeekValuePanel    _dayOfWeekPanel    /* =null */;
    ...
    protected JComponent layoutDayOfWeekComponent (Element elem, boolean useElement)
    {
        ...
        if (null == p)
        {
            if ((p=createDayOfWeekValuePanel(elem, useElement)) != null)
                setDayOfWeekValuePanel(p);
        }
        ...
        if (p != null)
        {
            ...
            if ((null == pos) || BorderLayoutPosition.CENTER.equals(pos))
                throw new IllegalStateException("layoutComponent(" + DOMUtils.toString(elem) + ") illegal day-of-week panel position: " + pos);
    ...
    protected JComponent layoutDayOfMonthComponent (Element elem, boolean useElement)
    {
        ...
        if (null == p)
        {
            if ((p=createDayOfMonthPanel(elem, useElement)) != null)
                setDayOfMonthValuePanel(p);
        }
