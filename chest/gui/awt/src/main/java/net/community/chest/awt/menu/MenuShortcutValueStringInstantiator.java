...
public class MenuShortcutValueStringInstantiator extends AbstractXmlValueStringInstantiator<MenuShortcut> {
	...
	public static final String toString (final MenuShortcut inst)
	{
		...
		if ((null == ks) || (ks.length() <= 0))
			throw new IllegalArgumentException("toString(" + inst + ") unknown key: " + k);

		if (inst.usesShiftModifier())
			return "SHIFT+" + ks;
		else
			return ks;
	}
	...
	public static final MenuShortcut fromString (final String v)
	{
		...
		if (null == k)
			throw new IllegalArgumentException("fromString(" + s + ") unknown key code");
		if (sPos < 0)
			return new MenuShortcut(k.intValue());

		final String	mod=(sPos < 0) ? null : s.substring(0, sPos);
		if (!"SHIFT".equalsIgnoreCase(mod))
			throw new IllegalArgumentException("fromString(" + s + ") unknown shift state");
	...
	public static final MenuShortcutValueStringInstantiator	DEFAULT=
								new MenuShortcutValueStringInstantiator();
}
