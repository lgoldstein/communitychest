...
public abstract class AbstractRrdPollerInstantiator {
	...
	public static final Collection<Collection<MBeanRrdDef>> updatePolledGroups (
			final Collection<Collection<MBeanRrdDef>> 	org,
			final Collection<? extends MBeanRrdDef>		grp)
	{
		if ((null == grp) || (grp.size() <= 0))
			return org;
		...
		for (final MBeanRrdDef mbd : grp)
		{
			...
			MBeanRrdDef		prev=
				((null == mbName) || (mbName.length() <= 0) || (null == grpMap) || (grpMap.size() <= 0)) ? null : grpMap.get(mbName);
			// if found previous instance, use a separate "group"
			if (prev != null)
			{
				if (null == res)
					res = new LinkedList<Collection<MBeanRrdDef>>();

				res.add(Arrays.asList(mbd));
				continue;
			}

			// if no previous instance, add it to the "main" group
			if (null == grpMain)
				grpMain = new LinkedList<MBeanRrdDef>();
			grpMain.add(mbd);
			...
			if (null == grpMap)
				grpMap = new TreeMap<String,MBeanRrdDef>(String.CASE_INSENSITIVE_ORDER);
	...
	public static final Collection<? extends Collection<? extends MBeanRrdDef>> groupPolledDefinitions (final Collection<? extends MBeanRrdDef> defs)
	{
		...
		final LongsMap<Collection<MBeanRrdDef>>	map=	// NOTE we rely on erasure here
			(LongsMap) new LongsMap<Collection>(Collection.class, numDefs, 4 /* no growth expected */);
		...
		for (final Collection<MBeanRrdDef> g : groups)
			res = updatePolledGroups(res, g);
	 ...
	 */
	public Collection<? extends Map.Entry<String,? extends AbstractMBeanRrdPoller>> start (
					final Collection<? extends MBeanRrdDef> defs,
					final ThreadFactory						tf,
					final LoggerWrapper						l) throws Exception
	{
		final Collection<? extends Collection<? extends MBeanRrdDef>>			gpList=
				groupPolledDefinitions(defs);
		final int																numGroups=
			(null == gpList) ? 0 : gpList.size();
		final Collection<Map.Entry<String,? extends AbstractMBeanRrdPoller>>	res=
			(numGroups <= 0) ? null : new ArrayList<Map.Entry<String,? extends AbstractMBeanRrdPoller>>(numGroups);
		if (numGroups > 0)
		{
			for (final Collection<? extends MBeanRrdDef> tdl : gpList)
			{
				if ((null == tdl) || (tdl.size() <= 0))
					continue;
				...
				try
				{
					...
					final Thread	t=(null == tf)
						? new Thread(te.getValue(), te.getKey())
						: tf.newThread(te.getValue())
						;
					if (null == t)	// unlikely...
						continue;
					t.start();
