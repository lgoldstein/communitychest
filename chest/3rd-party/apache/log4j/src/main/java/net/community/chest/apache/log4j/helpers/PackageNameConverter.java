...
public class PackageNameConverter extends PatternConverter {
    ...
    {
        final String    nm=(null == event) /* should not happen */ ? null : event.getLoggerName();
        final int        nmLen=(null == nm) /* should not happen - except for root */ ? 0 : nm.length();
        if (nmLen <= 0)
            return (null == nm) ? "" : nm;

        final int    lastPos=nm.lastIndexOf('.');
        if ((lastPos <= 0) || (lastPos >= (nmLen-1)))
            return nm;
