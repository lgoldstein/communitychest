...
public class RemoteRepository extends LocalRepository {
    ...
    /*
     * @see net.community.chest.apache.ant.mvnsync.LocalRepository#setPath(java.lang.String)
     */
    @Override
    public void setPath (String path)
    {
        if (haveRefId())
            throw new IllegalStateException("setPath(" + path + ") already using ref-id");
    ...
    public void setUrl (String url)
    {
        ...
        if ((uLen > 0) && (url.charAt(uLen-1) != '/'))
            throw new IllegalArgumentException("URL does not end in '/'");
    ...
    {
        if (haveRefId())
            throw new IllegalStateException("setId(" + id + ") already using ref-id");
    ...
    public void setRefId (String refId)
    {
        ...
        for (final String v : vals)
        {
            if ((v != null) && (v.length() > 0))
                throw new IllegalStateException("setRefId(" + refId + ") other attributes already set");
        }
    ...
    {
        final String    dGroup=d.getGroupId(),    // TODO add support for legacy layout(s)
                        grpSubPath=dGroup.replace('.', '/');
        // TODO add support for systemPath
